...
UNITTEST_SUITE_BEGIN(test_resource_pool)
{
    UNITTEST_FIXTURE(small_float)
    {
        ...
        UNITTEST_TEST(uintToFloat)
        {
            ...
            {
                u32 roundUp   = ncore::ngfx::nfloat::uintToFloatRoundUp(i);
                u32 roundDown = ncore::ngfx::nfloat::uintToFloatRoundDown(i);
            ...
            for (u32 i = 0; i < sizeof(testData) / sizeof(NumberFloatUpDown); i++)
            {
                ...
                u32               roundUp   = ncore::ngfx::nfloat::uintToFloatRoundUp(v.number);
                u32               roundDown = ncore::ngfx::nfloat::uintToFloatRoundDown(v.number);
        ...
        UNITTEST_TEST(floatToUint)
        {
            ...
            for (u32 i = 0; i < preciseNumberCount; i++)
            {
                u32 v = ncore::ngfx::nfloat::floatToUint(i);
                CHECK_EQUAL(i, v);
            }
            ...
            {
                u32 v         = ncore::ngfx::nfloat::floatToUint(i);
                u32 roundUp   = ncore::ngfx::nfloat::uintToFloatRoundUp(v);
                u32 roundDown = ncore::ngfx::nfloat::uintToFloatRoundDown(v);
                CHECK_EQUAL(i, roundUp);
                CHECK_EQUAL(i, roundDown);
                // if ((i%8) == 0) printf("\n");
                // printf("%u->%u ", i, v);
    ...
    UNITTEST_FIXTURE(offset_allocator)
    {
        ...
        UNITTEST_TEST(allocate_simple)
        {
            ...
            ncore::ngfx::allocation_t a = allocator->allocate(0);
            CHECK_EQUAL(0, a.offset);

            ncore::ngfx::allocation_t b = allocator->allocate(1);
            CHECK_EQUAL(0, b.offset);

            ncore::ngfx::allocation_t c = allocator->allocate(123);
            CHECK_EQUAL(1, c.offset);

            ncore::ngfx::allocation_t d = allocator->allocate(1234);
            ...
            ncore::ngfx::allocation_t validateAll = allocator->allocate(1024 * 1024 * 256);
        ...
        {
            // Free merges neighbor empty nodes. Next allocation should also have offset = 0
            ncore::ngfx::allocation_t a = allocator->allocate(1337);
            CHECK_EQUAL(0, a.offset);
            allocator->free(a);

            ncore::ngfx::allocation_t b = allocator->allocate(1337);
            CHECK_EQUAL(0, b.offset);
            allocator->free(b);
        ...
        UNITTEST_TEST(reuse_trivial)
        {
            ...
            ncore::ngfx::allocation_t a = allocator->allocate(1024);
            CHECK_EQUAL(0, a.offset);

            ncore::ngfx::allocation_t b = allocator->allocate(3456);
            CHECK_EQUAL(1024, b.offset);

            allocator->free(a);

            ncore::ngfx::allocation_t c = allocator->allocate(1024);
        ...
        UNITTEST_TEST(reuse_complex)
        {
            ...
            ncore::ngfx::allocation_t c = allocator->allocate(2345);
            CHECK_EQUAL(1024 + 3456, c.offset);

            ncore::ngfx::allocation_t d = allocator->allocate(456);
            CHECK_EQUAL(0, d.offset);

            ncore::ngfx::allocation_t e = allocator->allocate(512);
            CHECK_EQUAL(456, e.offset);

            ncore::ngfx::storage_report_t report = allocator->storageReport();
        ...
        UNITTEST_TEST(zero_fragmentation)
        {
            ...
            ncore::ngfx::allocation_t allocations[256];
            for (u32 i = 0; i < 256; i++)
            {
            ...
            u32 const no_space = ncore::ngfx::allocation_t::NO_SPACE;
            ...
            ncore::ngfx::storage_report_t report2 = allocator->storageReport();
